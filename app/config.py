...
class Config:
    ...
    # ─── سقف Groq الفعلي على خطة on_demand (المجانية) ────────────────
    # القيمة من رسالة خطأ Groq نفسها ("Limit 8000") — نفس الرقم لكلا
    # نموذجَي gpt-oss (120b و20b) على هذا الحساب حالياً، فالتبديل لموديل
    # أصغر عند التجاوز (GROQ_FALLBACK_MODEL) لا يحل مشكلة حجم الطلب
    # نفسها — فقط تصغير ما يُرسَل فعلياً يحلها (انظر fit_within_tpm_budget
    # بـai_service.py). لو رفعتم الحساب لخطة Dev Tier المدفوعة عند Groq
    # مستقبلاً، ارفعوا هذا الرقم ليطابق الحد الجديد.
    GROQ_TPM_LIMIT = 8000
    GROQ_TPM_SAFETY_MARGIN = 400   # هامش لعدم دقة تقدير التوكنات (تقريب بالحروف لا tokenizer حقيقي)
    GROQ_MIN_OUTPUT_TOKENS = 512   # أقل max_tokens نسمح بالنزول له تلقائياً — تحته الرد يفقد فائدته
    ...
    # ⚠️ 16000 هنا سقف أعلى فقط، لا قيمة مضمونة فعلياً: نظام برومبت
    # coder وحده تقريباً 1500-1900 توكن — بمجموع مع 16000 هذا يتجاوز
    # GROQ_TPM_LIMIT (8000) لوحده حتى بلا أي سياق محادثة، وكان هذا يعني
    # فشل وضع coder بخطأ 413 في *كل* رسالة قبل هذا الإصلاح. الآن
    # fit_within_tpm_budget بـai_service.py يخفّض القيمة الفعلية
    # المُرسَلة تلقائياً بما يناسب حجم السياق الفعلي بكل طلب — القيم
    # هنا تبقى الحد الأعلى المرغوب لا الفعلي المضمون. لإخراج أكبر فعلياً
    # لمشاريع ضخمة، الحل الوحيد الحقيقي ترقية حساب Groq لـDev Tier
    # (رابط بنفس رسالة خطأ 413: console.groq.com/settings/billing).
    MAX_TOKENS_MAP = {
